...
class ItemWithRangeKey(DynModel, name=None, validate_assignment=True, coerce_numbers_to_str=True):
    dy_client: ClassVar[ItemWithRangeKeyClient]
...
class ObjTestDef(ObjTestValues):
    get_via_id: Union[dict, str, ItemWithRangeKeyForStr, _internal.DynKey] = None
...
def compute_dyn_key_from_test_values(values: ObjTestValues) -> DynKey:
    hash_value, range_value = compute_key_serialization_from_test_values(values)
    return _internal.DynKey(client=values.model_cls.dy_client, hash_key=hash_value, range_key=range_value)
...
def simple_obj(simple_obj_def) -> 'ItemWithRangeKey':
    ...
    assert len(list(simple_obj_def.model_cls.dy_client.get())) == 1, "Have incorrect count."
    return obj
...
def simple_obj_second(simple_obj_def) -> 'ItemWithRangeKey':
    ...
    assert len(list(simple_obj_def.model_cls.dy_client.get())) == 2, "Have incorrect count."
    return obj2
...
def simple_obj_get_via_id(simple_obj, simple_obj_def) -> Optional[ItemWithRangeKey]:
    ...
    if isinstance(get_via_id, DynModel):
        ...
        obj = simple_obj_def.model_cls.dy_client.get_first(get_via_id)
...
def simple_obj_get_with_read_consistency(simple_obj, simple_obj_def) -> Optional[ItemWithRangeKey]:
    ...
    if isinstance(get_via_id, DynModel):
        ...
        obj = simple_obj_def.model_cls.dy_client.get_first(get_via_id, consistent_read=True)
        params = obj.dy_client.last_paginate_params
        assert (
...
def test_basic_dyn_class():
    ...
    fields = ItemOnlyHash.dy_client.dyn_fields
    expected_fields = {
...
def test_basic_delete(simple_obj, simple_obj_get_via_id, simple_obj_def):
    ...
    value = simple_obj_def.model_cls.dy_client.get_first(simple_obj.dy_id)
...
def test_getting_multiple_obj(simple_obj, simple_obj_second, simple_obj_def):
    ...
    model_cls.dy_client.put([obj1, obj2])

    # Verify we can lookup all objects in table.
    objs = list(model_cls.dy_client.get())
    ...
    objs = list(model_cls.dy_client.get(query={"hash_field": hash2}))
    ...
    range_result = list(model_cls.dy_client.get({
        'hash_field': simple_obj_other.hash_field,
        'range_field__range': [simple_obj_other.range_field, simple_obj_other.range_field]
    }))

    single_obj_result = list(model_cls.dy_client.get({
        'hash_field': simple_obj_other.hash_field,
...
def test_send_obj_with_related_child_and_sub_obj(simple_obj_values):
    ...
    o_gotten = ItemOnlyHash.dy_client.get_first("test-id3")
...
def test_pagination(simple_obj_values):
    ...
    if model_cls.dy_client.hash_key_info.py_type is str:
        for x in range(15):
    ...
    result = list(model_cls.dy_client.get())
...
def test_scan_raise_exception():
    with pytest.raises(NotImplementedError, match='There are no hash-keys'):
        ItemWithRangeKeyForStr.dy_client.get({'name': 'hello'})


def test_scan_with_read_consistency_works(simple_obj, simple_obj_def):
    # Just see if a simple scan works without problems
    result = list(simple_obj_def.model_cls.dy_client.get(consistent_read=True))
    assert len(result) == 1
    assert result[0].dy_client.last_paginate_params['ConsistentRead'] is True
...
def test_see_if_read_consistency_used(test_input):
    client = ItemWithRangeKeyForStr.dy_client
    ...
    client = ItemWithRangeKeyForStrWithDefaultConsistent.dy_client
...
def test_scan_fallback():
    ...
    items = ItemWithRangeKeyForStr.dy_client.get({'name': 'second'}, allow_scan=True)
    items = list(items)
    assert len(items) == 1
    assert items[0].hash_field == 'other-h'
    assert 'ConsistentRead' not in items[0].dy_client.last_paginate_params
    assert 'ScanIndexForward' not in items[0].dy_client.last_paginate_params
...
def test_conditional_delete():
    ...
    o.dy_save()
    assert len(list(ItemWithRangeKeyForStr.dy_client.get())) == 2
    ...
    objs = list(ItemWithRangeKeyForStr.dy_client.get())
...
def test_conditional_put():
    ...
    with pytest.raises(DynamoConditionError, match=r"condition failed: \{'name': 'n1'\}",
                       check=lambda e: e.condition == {'name': 'n1'}):
        o.dy_client.put(o, condition={'name': 'n1'})

    assert ItemWithRangeKeyForStr.dy_client.get_first(o.dy_id).items is None

    o.items = [{'a': 3}]
    o.dy_save(condition={'name': 'n2'})

    assert ItemWithRangeKeyForStr.dy_client.get_first(o.dy_id).items == [{'a': '3'}]
...
def test_reverse():
    ...
    items = ItemWithRangeKeyForStr.dy_client.get({'hash_field': 'hash'})
    items = list(items)
    assert len(items) == 2
    assert items[0].range_field == 'range-a'
    assert 'ScanIndexForward' not in items[0].dy_client.last_paginate_params

    rev_items = ItemWithRangeKeyForStr.dy_client.get({'hash_field': 'hash'}, reverse=True)
    rev_items = list(rev_items)
    assert len(rev_items) == 2
    assert rev_items[0].range_field == 'range-b'
    assert 'ScanIndexForward' in rev_items[0].dy_client.last_paginate_params

    item = ItemWithRangeKeyForStr.dy_client.get({'hash_field': 'hash', 'range_field': 'range-a'}, reverse=True)
    item = list(item)
    assert len(item) == 1
...
def test_multiple_hash_fields():
    ...
    MultiHashFieldModel.dy_client.put(o1)

    o1_via_get = list(MultiHashFieldModel.dy_client.get())[0]
